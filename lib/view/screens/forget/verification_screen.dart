...
class VerificationScreenState extends State<VerificationScreen> {
  ...
  void dispose() {
    ...
    // Regresar a la pantalla anterior para modificar el número
  void _modifyNumber() {
            ...
            return Column(children: [
              ...
              Get.find<SplashController>().configModel!.demo! ? Text(
                ...
                text: TextSpan(
                  children: [
                    TextSpan(
                      text: widget.fromSignUp ? 'enter_the_verification_sent_to_whatsapp'.tr : 'enter_the_verification_sent_to_email'.tr,
                ...
                TextButton(
                  onPressed: _seconds < 1 ? () {
                    if(widget.fromSignUp) {
                      authController.login(_number, widget.password , reeviarSms).then((value) {
                        if (value.isSuccess) {
                          _startTimer();
              ...
              // Botón para modificar el número
              Padding(
                padding: const EdgeInsets.symmetric(horizontal: Dimensions.paddingSizeLarge),
                child: CustomButton(
                  buttonText: _seconds > 0 ? '${'modify_number'.tr} ($_seconds)' : 'modify_number'.tr,
                  onPressed: _seconds > 0 ? null : _modifyNumber,
                ),
              ),
              const SizedBox(height: Dimensions.paddingSizeDefault),
              // Botón "No tengo WhatsApp" si fromSignUp es true
              widget.fromSignUp ? Padding(
                padding: const EdgeInsets.symmetric(horizontal: Dimensions.paddingSizeLarge, vertical: Dimensions.paddingSizeSmall),
                child: CustomButton(
                  buttonText: _seconds > 0 ? '${'no_whatsapp'.tr} ($_seconds)' : 'no_whatsapp'.tr,
                  onPressed: _seconds > 0 ? null : () {
                    const reeviarSms = true;
                    authController.login(_number, widget.password, reeviarSms).then((value) {
                      if (value.isSuccess) {
                        showCustomSnackBar('send_sms_success'.tr, isError: false);
                      } else {
                        showCustomSnackBar(value.message);
                      }
                    });
